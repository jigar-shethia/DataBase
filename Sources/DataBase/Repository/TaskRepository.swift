...
public final class TaskRepositoryImplementation :TaskRepository {
    ...
    public func get(isCompleted: Bool) -> AnyPublisher<Result<[Task], TaskRepositoryError>,Never> {
    ...
    public func add(task: Task) -> AnyPublisher<Result<Bool,TaskRepositoryError>,Never>{
    ...
    public func update(task: Task) -> AnyPublisher<Result<Bool,TaskRepositoryError>,Never> {
        let fetchRequest = TaskEntity.fetchRequest()
    ...
    public  func delete(task: Task) -> AnyPublisher<Result<Bool,TaskRepositoryError>,Never> {
        let fetchRequest = TaskEntity.fetchRequest()
        fetchRequest.predicate = NSPredicate(format: "id == %@", task.id as CVarArg)
        do{
